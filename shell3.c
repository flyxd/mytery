        ...
        {  
            ...
            {  
                ...
                {  
                    ...
                    //add_history(arg[0]);  //暂时无add_history
                }  
                else  
                {  
                    redirect(arg[0],i);  
...
{  
    ...
    int i,j,k,is_bg=0;  
    int yan_cmd=0,fd[10][2],pipe_in=-1,pipe_out=-1,flag=0;  //fd[10][1]
    pid_t pid;  
...
{  
    ...
    if (head!=NULL) {   /*链表不为空*/  
        do {  
            printf("%d  %d  %s/t%s/n",i,p->pid,p->state,p->cmd);  
            i++;  
            p=p->link;  
        }while (p!=NULL);  
    } else printf("No jobs!/n");/*链表为空*/  
}  
